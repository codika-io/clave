...
**`--test-no-activate`** (`src/main/test-mode.ts`) is the main-process flag that stops a test instance stealing the machine's focus: macOS accessory policy, no Dock icon, every window shown with `showInactive()`. `tests/e2e/harness.mjs` passes it on every launch, so under E2E `BrowserWindow.getFocusedWindow()` can be null and `win.isFocused()` false — assert Clave-internal focus, never the window manager's. It defaults off and off it changes nothing.
