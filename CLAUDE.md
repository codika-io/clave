...
- **`.clave` schema sync rule**: the schema is mirrored in **four** places that do NOT import each other, plus one separate repo. When you change an enum — or add, rename, or remove ANY `.clave` field — all five move in the same change:
  ...
  2. `src/main/ipc-handlers/clave-file-handlers.ts` — the main-process parser and writer (`ClaveGroupData`, `ClaveFileRaw`, `ClaveFileWriteData`, `resolveGroup`, `serializeGroup`).
  3. `src/preload/index.d.ts` — the typed IPC boundary (`ClaveFileGroupData`, `ClaveFileWriteData`). Easy to miss because nothing references it by name, and the read path silently loses the field if it lags.
  4. `src/main/mcp/mcp-server.ts` — the MCP tool schemas agents call.
  5. The `/clave:create-workspace` skill in the separate [`codika-io/clave-plugin`](https://github.com/codika-io/clave-plugin) repo — the reference agents author `.clave` files from. It needs its own PR, and it must not land after the app: a stale skill fails silently.

  Then check the round-trip, which is where a new field actually goes missing: `resolveGroup` (read), the sync writer and `exportClaveFile` in `pinned-store.ts` (write), `createPinnedFromGroup` AND `pinGroupFromCurrent` (both directions of pin ↔ live group), and the **single-group normalisation inside `importClaveFile`**, which rebuilds the object field by field and dropped a new field twice during PRDCT-1665.
