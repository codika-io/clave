...
function buildServer(callerSessionId: string | undefined): McpServer {
  ...
  server.registerTool(
    ...
    {
      description:
        'Query Clave\'s transport-layer exchange capture: what Clave recorded about inter-agent coordination — every clave_send_to_session delivery (timestamp, sender and target tab, full text, provenance header), agent-initiated tab spawns with their launch prompts, and Task-subagent fan-outs (discovered lazily from transcripts: recorded from the first delivery or query touching their parent session). Scope by group OR session, exactly one. Three views: "exchanges" (the recorded event timeline, with both endpoints\' token snapshots on each delivery — events come back AS STORED, v/kind/ts envelope included, so a record export is JSON.stringify per returned event: a faithful JSON value of the stored line, not a byte-identity claim), "usage" (each in-scope session\'s current token snapshot), "conversation" (the human-layer exchange: the human\'s messages plus the agent\'s text blocks with operations stripped — no tool calls, no tool results, no thinking — each agent entry tagged mid-turn or end-of-turn). Every conversation entry also carries `origin`, because a transcript stores an inter-agent delivery as a user entry and `role` alone cannot tell them apart: "human" (the human typed it), "agent-transport" (it ARRIVED from a sibling tab via clave_send_to_session — it carries Clave\'s provenance header, so no human wrote it), "system" (a Claude Code control marker such as an interrupt notice), and "agent" on every assistant entry. `role` keeps its previous meaning, and the classification is labelling only: no entry is dropped or reordered, and since/limit/turn tags are unaffected. Token snapshots carry two DISTINCT numbers, never conflate them: `billed` is cumulative spend — input/output/cache-creation/cache-read tokens summed over every API call in the session\'s transcript, Task-subagent sidecars INCLUDED (their share broken out under billed.subagents) — while `contextOccupancy` is how full the root session\'s context window is right now (the tokens that entered its latest completed call; sidecars excluded). Snapshots read the transcript as it is on disk: a still-streaming turn shows the last completed call. Read-only: nothing about the queried sessions is touched. Access uses clave_read_session\'s relationship gate — group scope requires your tab to be a member of that group; session scope requires spawn lineage ("parent", a tab you opened) or a shared group; anonymous callers are refused. The durable read path for anyone outside those relationships is the workstream record the capture exports to, not this live tool.',
      inputSchema: {
        group: z
          .string()
          .optional()
